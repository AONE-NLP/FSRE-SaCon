        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if step % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [0, -1] and global_step % args.save_step == 0:
                    ...
                    torch.save(ckpt_bert1, os.path.join("../ckpt/ckpt_sacon_bert1", "ckpt_of_step_"+str(global_step)))
                    torch.save(ckpt_bert2, os.path.join("../ckpt/ckpt_sacon_bert2", "ckpt_of_step_"+str(global_step)))
...
if __name__ == "__main__":
    ...
    # Model and dataset
    if args.model == "SaCon":
        model = SaCon(args).to(args.device)
        train_dataset = SaConDataset("../data/SaCon", args)
    else:
        raise Exception("No such model! Please make sure that `model` takes the value in {SaCon}")
